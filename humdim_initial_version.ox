...
initialize() {
  ...
  rentinc=new matrix[NP][1];//aggregate resource rents
  permitinc=new matrix[NP][1];//aggregate permit rents
  dividends=new matrix[NP][1];//total dividends (in equilibrium, number of shares sum to one, so it will be per share
  shareprice=new matrix[NP][1];//total dividends (in equilibrium, number of shares sum to one, so it will be per share
  
  phi=new matrix[NP][1];
  theta=new matrix[NP][1];
return 1;
}
...
rho[][]=1;
  ...
  for(i=1;i<NP;++i)
    {
    phi[i][0]=phi[i-1][0].*(1+gammaphi*(1-deltaphi)^(i));
	Omegat[i][0]=Omegat[i-1][0].*(1+gammao*(1-deltao)^(i));
	theta[i][0]=theta[i-1][0]*(1+gammath*(1-deltath*deltath2^i)^i);
	}
...
datacheck(j,NPP) {
	...
	for(j=0;j<60;++j)	{
		i=j+9;
		println(1962+i,"  ",GDP[i][0],"    ",data[j][gdpdat],"    ",E[i][0]*phi[i][0],"   ",data[j][energydat],"   ",E[i][0],"   ",data[j][emitdat],"   ",cstate[i][atm1],"   ",data[j][atmco2dat]);
		errors[j][0]=(GDP[i][0]-data[j][gdpdat])./data[0][gdpdat];
		errors[j][1]=(phi[i][0]*E[i][0]-data[j][energydat])./data[0][energydat];//get to same order of magnitude as GWP - use first period levels
		errors[j][2]=(phi[i][0]-data[j][energydat]/data[j][emitdat])./(data[0][energydat]/data[0][emitdat]);
		//errors[j][1:2].*=1;
		}
...
calibprint() {
	decl i;
	for(i=1;i<NP;++i)
		println(i,"  ",Omegat[i][0],"   ",Omega[i][0],"   ",cstate[i][],"     ",E[i][0],"     ",CumC[i][0],"     ",mextcost[i][0],"     ",E[i][0]*phi[i][0],"       ",L[i][0],"      ",sumr(pop[i][])[0][0],"      ",pop[i][0],"   ",K[i][0],"   ",U[i][0],"    ",GDP[i][0],"    ",cons[i][0],"   ",sumr(cons[i][].*pop[i][0]*10^(-6))[0][0],"   ",phi[i][0],"   ",theta[i][0],"   ",prices[i][0],"   ",prices[i][1],"   ",prices[i][2],"   ",prices[i][3],"   ",P[i][0],"   ",Pfirm[i][0],"   ",ctax[i][0],"   ",rentinc[i][0],"   ",permitinc[i][0],"   ",dividends[i][0],"   ",income[i][0],"   ",permit_transfers[i][0]);
		}
...
invest_nl(sysval,capital) {
	...
	decl capchoice=(K[0][0]|sent[][0])~(0|sent[][1]);
	...
	dividends[NP-1][0]-=capchoice[][0][NP-1][0]./(1-delta); //force firm to maintain capital stock.

	//use .1 here - as long as it is not binding in equil it is fine...using exactly zero admits small neg values.
	kt=(dividends[:NP-2][0].<=0.1).*(dividends[:NP-2][0]-0.1)+(dividends[:NP-2][0].>0.1).*capchoice[1:][1]+(capchoice[1:][1].>0).*capchoice[1:][1];
...
extract_nl(sysval,sent_vals) {
	...
	decl costs=10^(-3)*xi[0][0]*extraction+10^(-3)*(xi[1][0]*6000/(xi[2][0]+1))*(((Cumtemp+extraction)/6000).^(xi[2][0]+1)-((Cumtemp)/6000).^(xi[2][0]+1));
	//calculate the change in extraction costs for a small increase in extraction
	decl costs_eps=10^(-3)*xi[0][0]*extraction+10^(-3)*(xi[1][0]*6000/(xi[2][0]+1))*(((Cum_eps+extraction)/6000).^(xi[2][0]+1)-((Cum_eps)/6000).^(xi[2][0]+1));
	...
	mextcost=10^-3*(xi[0][0]+xi[1][0].*((Cumtemp[][0]+extraction)/6000).^xi[2][0]);//marginal extraction cost
...
agents_problem(itermax)	{
	...
	sp_ext[0][0]=shareprice[NP-1]*prices[NP-1][equity]-dividends[NP-1][0];
	for(step=1;step<NG;++step)  {
	  sp_ext[step][0]=sp_ext[step-1]*prices[NP-1][equity]-dividends[NP-1][0];
 	  }//end for loop													   
...
equil(file_load,file_save) {
	...
	while(crit_equil>10^(-7))	{
		...
		while(firm_crit>10^(-6)) {									
			...
			extcost=10^(-3)*xi[0][0]*X+10^(-3)*(xi[1][0]*6000/(xi[2][0]+1))*(((CumC+X)/6000).^(xi[2][0]+1)-((CumC)/6000).^(xi[2][0]+1));
		...
		for(timet=NP-2;timet>=0;--timet)  {
		  shareprice[timet][0]=(shareprice[timet+1][0]+dividends[timet+1][0])/prices[timet+1][equity];
	 	  }//end for loop
		agent_converge=0;
		agent_crit=10^(-8);
