...
//	X=loadmat(indir+"Xfile"~file_load~".dat");
    ...
    do {
		...
		do {//solve extraction and production given rate of return
			...
			if (X_converge) println("ext",ext_euler');
			print_equil=0;
			...
			if (K_converge) println("inv ",inv_euler');
			print_equil=0;
			...
			firm_crit = norm(X-xold,2)+ norm(K-kold,2);  //CF almost same criterion
//			firm_crit=meanc(meanr(((K~X)-(kold~xold))).^2); //mean square deviation
//CF:redundant			xold=X;
//						kold=K;
			println("Firm Convergence crit, X, K ",firm_crit,"   ",X_converge,"   ",K_converge);
		...
		shareprice=constant(1000,D::P,1);	//the price of one share in the last period in dollars
		...
		do {
            ...
            if (agent_converge>0)  println("Unconverged assets - recalculating");
			} while(agent_converge>0);
		  adj= 1E-6 * sumr(assets.*pop) - 1;  //CF: simplified
		  adj[][]=spline(adj,range(1,D::P)',0);//locally smooth adjustment - keeps it from going pos/neg/pos
